...
def main(outdir):
    ...
    df_all_elements.to_csv(f"{outdir}/TAT_defense_results/simulation/0.1-Elements_genomic_features_all_IS_families.tsv", sep = "\t", index = False)
    ...
    for elem_family in tqdm(list_IS_families_to_simulate, desc = "Running the simulations for the IS genes (either all, or based on their family)", total = len(list_IS_families_to_simulate)):
        ...
        df_elem_family_with_defense = convert_spot_to_number_of_elements_simulated(df_simulations_IS, df_all_elements[df_all_elements["Element_type"] == "TA"], elem_family)
        df_elem_family_with_defense.to_csv(f"{outdir}/TAT_defense_results/simulation/data/3.{n}.2-Colocalization_perspectives_from_TA_point_of_view_with_{elem_family}.tsv", sep = "\t")
        ...
        tmp_global_res = plot_mean_distribution_from_perspective(df_elem_family_with_TA,
                                                                ...
                                                                elem_family,
                                                                "TA",
                                                                f"{outdir}/TAT_defense_results/simulation/plot/3.{n}.3_Random_distribution_of_{elem_family}_from_TA_perspective.png")
...
def plot_mean_distribution_from_perspective(df_sim, df_real, family_colocalizing, elem_perspective_name, outname):
    ...
    # Texte indicatif
    plt.text(0.95, 0.8,
             f"Simulations > real : {n_above} ({round((n_above + n_equal)/len(simulation_means)*100, 2)}%)\n"
    ...
    return [family_colocalizing, str(real_mean), str(n_below), str(n_equal), str(n_above), str(round(p_value_lower,2)), str(round(p_value_higher,2))]
...
def init_global_res_file(outdir):
    ...
    with open(outdir + "/TAT_defense_results/simulation/data/3.0-All_IS_families_colocalization_from_TA_perspectives.tsv", "w") as f:
        f.write("IS_family\tN_members_in_family\treal_mean_value\tn_below_real_mean_value\tn_equal_real_mean_value\tn_over_real_mean_value\tp-value_below_equal_real\tp-value_over_equal_real\n")
...
def write_global_res(res2add, outdir):
    ...
    with open(outdir + "/TAT_defense_results/simulation/data/3.0-All_IS_families_colocalization_from_defense_perspectives.tsv", "a") as f:
